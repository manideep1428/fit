...
export default function EditGoalScreen() {
  ...
  useEffect(() => {
    if (goal) {
      ...
      if (goal.measurements && goal.measurements.length > 0) {
        ...
        setMeasurements([{ bodyPart: 'Stomach', current: '', target: '', unit: 'in' }]);
      }
    }
  }, [goal]);
  ...
  const handleSubmit = async () => {
    if (!goalDescription.trim() || !user?.id || !clientId || !goalId) {
      return;
    }
  ...
  const isGoalNameDuplicate = checkGoalNameUnique === false;
        ...
        <View className="mb-6">
          ...
          <TextInput
            ...
            style={{
              ...
              borderWidth: 1,
              borderColor: isGoalNameDuplicate ? '#EF4444' : colors.border,
              color: colors.text,
              ...shadows.small,
            }}
      ...
      >
        <TouchableOpacity
          ...
          style={{
            ...
            opacity: (!goalDescription.trim() || loading || isGoalNameDuplicate) ? 0.5 : 1,
          }}
          onPress={handleSubmit}
          disabled={loading || !goalDescription.trim() || isGoalNameDuplicate}
        >
          {loading ? (
            <ActivityIndicator color="#FFF" />
          ) : (
            <Text className="text-base font-bold text-white">Update Goal</Text>
          )}
        </TouchableOpacity>
      </View>
